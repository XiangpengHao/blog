...
The simplest way to do matrix multiplication is to dequantize the weights of both sides to fp32, then do the good old multiplication, and quantize the result back to the target format.

Before we judge the performance of this naive approach, let's first build a mental model of what performance we should expect. For two matrix [a, b] and [b, c], it's not too hard to see that:
```text
Optimal compute: 2 * a * b * c = 2abc
Optimal memory: ab + bc + ac
```

For our naive approach, since we need to first dequantize both sides to fp32, we need to read ab + bc bytes of memory, then write 8x that many bytes of memory, ac also needs to be dequantized, causing 5x ac bytes of memory, so the total memory is 9x(ab + bc) + 5xac.
The dequantization requires 2 flops per element, so the total flops is 2x(ab + bc + ac) + 2abc.
Clearly the naive approach is very wasteful, and we need a way to directly perform the matrix multiplication using the quantized weights. 

Before we perform the mutliplication, there's one more thing to consider: the format of the activation tensor, i.e., the `[1, 2048]` tensor (or `[n, 2048]` for prefill).
Unlike weights, which is pre-quantized in the file, we have the freedom to choose how to represent the activation tensor, we can do any of the floating point formats in @fig-float-formats as we wish.
A common choice is to use bf16, a format with good range.

Now we can narrow the porblem down to this challenges: **we have a block of `Q4_K` and a block of `bf16`, How do to efficiently dot product?**

The math is simple: $x = l[0] * r[0] + l[1] * r[1] + ... + l[i-1] * r[i-1]$, where `l` is the `Q4_K` block and `r` is the `bf16` block.

No hardware can directly manipulate the bits like this, so we need to nudge the format to match with the hardware.
One common efficient instruction is to use u8*i8->f32.
So we first need to convert the `bf16` to a i8 representation, i.e., quantize it to Q8. Since the left block has 256 weight, we'd also want right block to have 256 weights, so we choose `Q8_K`. 
Now that we know each r_i = q_r[i] * scale_r, and l_i = (q_l[i] * scale_l - min_l), so their mutiplication x_i = (q_l[i] * scale_l - min_l) * q_r[i] * scale_r = q_l[i] * q_r[i] * scale_l * scale_r - min_l * q_r[i] * scale_r.
This means that we can directly perform the mutiplication on the raw weights without any intermediate representation.

To summarize, here're the steps:

1. Quantize the `bf16` block to `Q8_K`.
2. Unpack `Q4_K` block to `u8` for alignment.
3. Perform the vectorized multiplication on the raw weights.
