...
However, each MoE block only activates 1/32 of the parameters, while rest of the parameters are activated all the time, so each token needs to activate 0.508M + 1.01B + 272.66M + 32.36B/32 + 0.508M ~= 3B parameters.
...
1. Pre-processing a string to setences.
2. Vocabulary lookup.
3. Merge rules
4. Special token handling, space, non-safe ascii values.


## From token to embeddings

## Read our first tensor
