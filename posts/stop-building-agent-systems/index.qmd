...
categories: []
description: "Build agent systems for human."
toc: true
comments:
  giscus: 
...
When a vibe-system fails, we can only hope LLMs will fix it by themselves. 
But systems that are unaccountable are useless, and will eventually collapse.
...
We don't have enough infrastructure to understand the systems we build, maintain, and rely on.
...
No one is able to reproduce a rare production bug, no one is able to fully explain why performance jitters, and no one is able to fully understand how systems interact.
