...
description: "Distributed caching with compute pushdown, compressed execution, and network-efficient data transfer."
categories: []
comments:
  giscus: 
...
[LiquidCache](https://github.com/XiangpengHao/liquid-cache) is designed to be the new caching infrastructure for the next decade of data analytics.[^5]

It seamlessly allows Parquet-based analytical systems to benefit from the latest data encodings and pushdown optimizations:

- At a glance, LiquidCache is a distributed caching service: it supports all object storage backends (S3, GCS, Azure Blob Storage, etc.), and serves all kinds of applications (knowledge bases, dashboards, etc) deployed on all kinds of compute (k8s, lambda, etc).
...
This blog post discusses why we build LiquidCache, and how LiquidCache works. 
...
![Compare the traditional on-premise architecture (left) with the new diskless+serverless architecture (right). LiquidCache is at the center of state management.](./state-management.png){width=100%}
